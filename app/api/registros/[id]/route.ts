...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // ID de mock — devolver datos hardcodeados sin tocar DB
    if (id.startsWith('m')) {
      const mock = mockRegistros.find(r => r._id === id);
      if (!mock) return NextResponse.json({ success: false, error: 'No encontrado' }, { status: 404 });
